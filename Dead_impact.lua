...
local Search = {
    { class = "ShopFreePurchaseSpendComponent", method = "IsSuitable" },
    { class = "CustomizationUtils", method = "IsUnlock" },
    { class = "ExperienceUtils", method = "GetExperienceMultiplierByLocationTierAndRank" },
    { class = "ReputationRestrictionsProvider", method = "IsEnoughReputation" },
    { class = "PlayerClassStatUtils", method = "GetAvailableStatPoints" },
    { class = "ItemLevelingUtils", method = "CalculateDonationExperience" },
}
...
-- ==============================
-- FUNCTION HIDE HISTORY
-- ==============================
function hideHistory()
    gg.clearResults()
    local allResults = gg.getResults(gg.getResultsCount())
    if #allResults > 0 then
        gg.removeListItems(allResults)
    end
    gg.toast("🛡 History value disembunyikan")
end
...
if H3 == OFF then  
         ...
         RC(0x523368)
        IF(GetExperienceMultiplierByLocationTierAndRank, 99999)
        
        H3 = ON 
        gg.toast("Experience ON")
    else
        OG(0x523368)
        gg.toast("Experience OFF")
        H3 = OFF
    end
end
...
if H4 == OFF then  
         ...
         RC(0x523368)
        I(IsEnoughReputation, 1)
        
        H4 = ON  
        gg.toast("Tier Equip ON")
    else
        OG(0x523368)
        gg.toast("Tier Equip OFF")
        H4 = OFF
    end
end
...
if H5 == OFF then  
         ...
         RC(0x523368)
        I(GetAvailableStatPoints, 500)
        
        H5 = ON
        gg.toast("Stat Point ON")
    else
        OG(0x523368)
        gg.toast("Stat Point OFF")
        H5 = OFF
    end
end
...
if H6 == OFF then  
         ...
         RC(0x523368)
        I(CalculateDonationExperience, 5000)
